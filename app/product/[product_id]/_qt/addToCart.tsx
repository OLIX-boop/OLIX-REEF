    ...
    <div className="mt-5 flex flex-col min-[1350px]:flex-row">
      <div className="w-min md:mx-0 mx-auto">
        ...
        <div className="addCart flex border-gray-200 w-min rounded-xl h-10">
          <button
            onClick={() => useQt((prev) => (prev >= stock ? prev : ++prev))}
            className="btn1 px-4 border-2 hover:border-black"
          >
            +
          </button>
          <input
            value={qt}
            className="w-14 focus:outline-none text-center border-t-2 border-b-2"
            type="number"
            name=""
            id=""
          />
          <button
            onClick={() => useQt((prev) => (prev <= 1 ? prev : --prev))}
            className="btn2 px-4 border-2 hover:border-black"
          >
            -
          </button>
        </div>
      ...
      <button
        className="md:mx-0 mx-auto cart-button min-[1350px]:ml-5 border-0 rounded-sm relative p-1 w-[60%] min-[1350px]:w-[40%] h-10 mt-4  min-[1350px]:mt-auto"
        onClick={handleCart}
      >
